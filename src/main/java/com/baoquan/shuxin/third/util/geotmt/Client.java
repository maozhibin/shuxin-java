 ...
 */
public class Client {
    private String server = "http://yz.geotmt.com:80"; // http://yz.geotmt.com、https://yz.geotmt.com
    private int encrypted = 1; // 是否加密传输
    private String encryptionType = "AES"; // AES(秘钥长度不固定)、DES(秘钥长度8)、DESede(秘钥长度24)
    private String encryptionKey = "sqkj123456"; // 加密类型和加密秘钥向GEO索取(如果是获取数据的时候传的是RSA那么这里自己定义即可)
    private String username = "sqkj"; // 账户向GEO申请开通
    ...
    private static final int httpConnectTimeout = 60000;
    private static final int httpReadTimeout = 60000;
    ...
    public String getData(String path, Map<String, String> params) {
        ...
        if (dsign == 1) {
            ...
            String digitalSignature = DigitalSignature.clientDigitalSignature(paraMap, null, "",
                    digitalSignatureKeyMap.get(username));
    ...
    public String getToken() {
        if (getNewToken()) {
            ...
            if ("RSA".equalsIgnoreCase(etype)) {
                ...
                params = "username=" + eUsername + "&password=" + ePassword + "&etype=" + etype + "&encryptionType="
                        + paraEncryptionType + "&encryptionKey=" + paraEncryptionKey + "&encrypted=" + encrypted
                        + "&dsign=" + eDsign;
            } else {
                params = "username=" + eUsername + "&password=" + ePassword + "&uno=" + uno + "&encrypted=" + encrypted
                        + "&dsign=" + eDsign;
            }
            ...
            String rs = HttpClient.getRs(path, params, reqencoding, respencoding, requestMethod, httpConnectTimeout,
                    httpReadTimeout, headerMap);
            // 解密返回值
            if (rs.startsWith("{")) {
    ...
    public boolean getNewToken() {
        Long tokenTime = getTokenTimeMap.get(username);
        long getTokenTime = tokenTime == null ? 0l : tokenTime;
        return tokenIdMap.get(username) == null || "".equals(tokenIdMap.get(username))
                || System.currentTimeMillis() - getTokenTime >= tokenCycle;
    }
